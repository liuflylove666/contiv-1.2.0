...
while getopts ":c:e:s:n:v:f:w:t:k:a:u:p:l:d:b:m:y:z:g:i:" opt; do
	case $opt in
	e)
...
sed -i.bak "s/__ROUTING__/$fwd_mode/g" $contiv_yaml
sed -i.bak "s/__NETMODE__/$net_mode/g" $contiv_yaml
...
# Install Contiv
cat $contiv_yaml > /tmp/contiv.yaml
...
if [ "$net_mode" == "vlan" ]; then
    netctl global set --fwd-mode bridge --fabric-mode default --vlan-range "1-4094"  && netctl net ls -q | grep -q -w "contivh1" || netctl net create -t  default -e vlan -p 100 -s 192.168.1.0/24 -g 192.168.1.254 contivh1
...
echo " For example, netctl net create -t default -e vlan -p 608 -s 20.1.1.0/24 -g 20.1.1.1 default-net "
echo " "
echo "========================================================="
